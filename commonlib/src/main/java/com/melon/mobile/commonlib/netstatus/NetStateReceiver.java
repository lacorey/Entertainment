package com.melon.mobile.commonlib.netstatus;
...
public class NetStateReceiver extends BroadcastReceiver {
    public final static String CUSTOM_ANDROID_NET_CHANGE_ACTION = "com.melon.mobile.commonlib.net.conn.CONNECTIVITY_CHANGE";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getAction().equalsIgnoreCase(ANDROID_NET_CHANGE_ACTION) || intent.getAction().equalsIgnoreCase(CUSTOM_ANDROID_NET_CHANGE_ACTION)) {
            if (!NetUtils.isNetworkAvailable(context)) {
                DLog.i(TAG, "<--- network disconnected --->");
                isNetAvailable = false;
            } else {
                DLog.i(TAG, "<--- network connected --->");
                isNetAvailable = true;
